...
if __name__ == '__main__':
    ...
    import_path = f'D:/PycharmProjects/GMM/data/sparse_training_data/'
    ob_points = 91
    total_input = import_path + f'{save_name}_input_{ob_points}_{ob_points}_fixed.csv'
    total_output = import_path + f'{save_name}_output_{ob_points}_{ob_points}_fixed.csv'

    # # load the both_ids, to separate the training data if needed
    # with open(f'data/sparse_training_data/both_ids_{save_name}.pkl', 'rb') as f:
    #     both_ids = pickle.load(f)

    # load the both and growth data separately
    both_input = import_path + f'{save_name}_input_{ob_points}_{ob_points}_fixed_both.csv'
    both_output = import_path + f'{save_name}_output_{ob_points}_{ob_points}_fixed_both.csv'
    growth_input = import_path + f'{save_name}_input_{ob_points}_{ob_points}_fixed_growth.csv'
    growth_output = import_path + f'{save_name}_output_{ob_points}_{ob_points}_fixed_growth.csv'

    # ----------------------------------------------

    X = pd.read_csv(growth_input, index_col=0)
    Y = pd.read_csv(growth_output, index_col=0)
    ...
    # ----------------------------------------------
    pd.DataFrame(y_est).to_csv(f'data/prediction_results/fixed_est_{save_name}_growth.csv')
    pd.DataFrame(y_test).to_csv(f'data/prediction_results/fixed_test_{save_name}_growth.csv')
